...
var def_aya = '1_1', // default displayed Aya
def_qaree = 'Hudhaify_64kbps', // default reciter
def_lang = 'ar', // default language
def_mosshaf = 'hafs', // default mosshaf
def_trans = '', // default trans (empty because it's based on the selected language)
def_tafsir = 'sa3dy', // default tafsir
def_vision = 'normal', // default vision (normal or night mode)
def_keeplight = "yes", // default keep the light when the app is open
def_keepworking = "yes", // default keep the app working in the background (not used, it was for some exprements only)
...
tafaser = {}, // declared here, filled in the switchLang function (should be modified there to add more tafaser)
quraa = {}, // declared here, filled in the switchLang function (should be modified there to add more tafaser)
...
// declare the current variables holders the will be used for holding current values globally
currMosshaf, currVision = 'normal', currTrans, currTafsir, currActiveWin, currPage = 0, page_key="Page", autoStop=false, 
currLang, currAya, isZoom, globalPlay, pausedByCall = 0, mp3url = '', repeat_started = 0,
/////


// declaration for global internal variables
pb_trans_map = { "En": "English_Walk", "Fr": "fr.leclerc_128kbs", "Ur": "ur.khan_46kbs", "Bs": "Bosnian_Korkut_128kbps" },
p_paused = null, // is player paused, defaults to null
_lang = false,
_first_run = false,
_emp = function () {},
oldAndroid = false,//(navigator.userAgent.indexOf("Android 2") != -1),
_FR_ = false, // variable that holds whether it's first time to run the app or not
langs_av = { "ar": { l_dir: "rtl" }, "en": { l_dir: "ltr" } },
scrW, scrH, prev_scrW, prev_scrH, // for screen width and height
in_mtest_mode = false;

SG = (typeof SG == "undefined") ? {} : SG; // super global SG is the holder for global variables
...
(function (window, $) {
    ...
    // declaration for the global variables inside the context //
    var imgs_t, imgs_t2, // images loading timers
    hilites = {}, pb_trans, pb_played, // hilights (x,y) positions holder

    trans_enb = 1, // is the translation enables? default:1
    transR = 0, // the translation window height ratio (relative to total screen height)

    /*---- local temp variables (used locally in the context) ----*/
    mosshafSeld, nav, nav_menu, nav_trans,
    t_ele, transH, transR,
    tdiff_ele = 0, TST = 0, cancelNextS,
    in_repeat_mode, aya_prev,
    tViews = 25, // number of parallel images
    inViews = 15, // numer of default active image
    CPX = prevS = inViews,
    al_audio, al_image,
    canHTM = null, // can hide tools menu
    nav_menu = false,
    navReseted = false,
    tc_moved = false,
    tc_timer = false,
    ...
    // the initer which do the boot
    SG.Initer = {
        // the init method
        init: function () {
            this.preInit(); // holds the actions that should be done before the initialization
            this.preHook(); // a hook for holding the actions that should be done before the initialization
            this.preHook2();// another hook for holding the actions that should be done before the initialization
            this.initRoot(); // initilize the root of the app (core funcionality)
            this.initMainWidg(); // initlize the main interface (main app window)
            this.initPlayerActions(); // initilize the audio player
            this.initToolsActions(); // initilize the tool bar
            this.initMenuActions(); // initilize the side menu
            this.postHook();  // a hook for holding the actions that should be done after the initialization
            this.postHook2(); // another hook for holding the actions that should be done after the initialization
            this.postInit(); // holds the actions that should be done after the initialization
        },
        preInit: function () { }, // declared for future use
        preHook: function () { },  // declared for future use
        preHook2: function () { },  // declared for future use
        
        /*---
        the root initialization
        ---*/
        initRoot: function () {
            if (!currLang) { // if currLang present means switchLang called, otherwise call the switchLang function
                switchLang(this.currLang || def_lang);
            }

            /*---- 
            init the popups, popups are modal widgets used to display tafasir and help contents
            ------*/
            ...
            /*--- END initializing popups ---*/

            /*--- a special css file for very old android (android 2.x) ---*/
            if (oldAndroid) {//
                $("head").append('<link type="text/css" href="css/oldandroid.css" rel="stylesheet">');
            }
            
            // fill the curr values
            ...
            SG._ap1 = new SG.APlayer(); // _ap1 is the instance object of the audio player class
            SG._ap1.onComplete = function () {} // // attatch the on complete handler to an empty method

        },
        /*------
        the main widg is the main application screen
        ------*/
        initMainWidg: function () {
            var frame = new Image();
            frame.src = 'frame_' + currMosshaf + '.jpg'; // the frame is displayed while the image is loading

            // lets fill the image list
            var str = '';
            ...
            if (applyAnd4Fix()) { // a special fix for android v4
                $("#scroller").addClass('and4fix');
            }

            SG.DimMan.adjust(); // calculate the dimentions
            ...
            // very old android (v2) displayes submenus in popups
            if (!oldAndroid) {
            ...
            // toggle the tools menu upon tap event
            ...
            $("#wrapper").on(END_EV, function (e) {
                ...
                if (didDT) { // if it was double tap, then do the zoom in/out
                    loadAya(currAya.split('_')[0], currAya.split('_')[1], true, true); // reload
                    SG.DimMan.refresh(false, true); // reset dimentions
                    didDT = false;
                    SG.ToolsMenu.toggle(-1);
                    var hn = getKey('chooseAya_hint') || 0;// show alert on how to select Aya
                    if (!hn || hn < 4) { // but the alert should be shown 4 times only
            ...
            /*---- handling the long tap ----*/
            var tdiff_ele = 0; // the 1st timer
            ...
            $("#wrapper").on(END_EV, ".hiliter.copy", function (e) {//
                ...
                if (!SG.Naver.get('wrapper').moved) {
                    ...
                    if (tdiff >= 250) { // if long tap
                        setCurrAya(tAya);
                        SG._ap1.onComplete = function () { };
                        SG._ap1.stop();// stop the player

                        canHTM = false; // can hide tools menu?
                        setTimeout(function () {
                            SG.ToolsMenu.toggle(1);
                            canHTM = true;
                            playAya();
            ...
            SG.Naver.get('wrapper').on("scrollEnd", function (obj) {
                if(in_mtest_mode){ // is in the test mode?
                    CPX = 0;
                    return; // then do nothing
                }
                
                if (cancelNextS) { // temp variable used as a work around for some logic flow
                    cancelNextS = false;
                    return; // do nothing
                }


                if (this.currentPage.pageX == prevS) return; // the movement still on the same page? then do nothing

                CPX = this.currentPage.pageX; // the curr pageX

                if (CPX > prevS) { // move backword
            ...
            SG.Naver.attach('trans', 'defaultSet'); // add the naver functionality to the translation pane.

            // when click on the small book icon in the upper tool bar show the choose sura submenu.
            ...
            // when click on the close icon, hide the translation pane
            ...
            // for increasing/decreasing translation font size
            $("#tc_img_zin,#tc_img_zout").on(END_EV, function (e) {
                e.preventDefault();
                e.stopPropagation();
                $(this).removeClass('active');
                var s = getKey("curr_tfs") || 17;//parseInt($("#trans_cont").css('font-size'));
                s = parseInt(s,10);
                //alert(s);
                s += ($(this).attr("id") == 'tc_img_zin') ? 1 : -1;
                //alert(s);
                setKey('curr_tfs', s);
                $("#trans_cont").css('font-size', s+"px");
            ...
            $("#trans_ctrl").on(START_EV, function (e) {
                ...
                var p1 = (hasTouch && e.originalEvent.touches && e.originalEvent.touches.length) ? e.originalEvent.touches[0] : e;
                //alert(2);
                e.preventDefault();
                e.stopPropagation();
                ...
                $(this).on(MOVE_EV, function (e2) {
                    e2.preventDefault();
                    e2.stopPropagation();
                    var p2 = (hasTouch && e2.originalEvent.touches && e2.originalEvent.touches.length) ? e2.originalEvent.touches[0] : e2;
        ...
        initToolsActions: function () {
            ...
            $("#share_twit").on(END_EV, function (e) {//
                ...
                SG.Plugins.DBP.search('id', aya_id, tash, function (json) {
                    ...
                    var url = 'https://twitter.com/home?status=' + encodeURIComponent(nass + "\n" + 'https://quran.ksu.edu.sa/index.php?aya=' + sura + '_' + aya);
            ...
            $("#share_fb").on(END_EV, function (e) {
                ...
                var subj = _lang['sura_s'] + ' ' + QuranData.Sura[sura][sura_key] + ' : ' + _lang['aya_s'] + ' ' + aya;
                var aya_id = aya2id(sura, aya);
                var url = 'https://facebook.com/sharer/sharer.php?u=' + encodeURIComponent('https://quran.ksu.edu.sa/index.php?aya=' + sura + '_' + aya + '&l=' + currLang + '#aya=' + sura + '_' + aya);//+'&p[title]='+escape(subj)+'&p[summary]='+escape(nass)+'&p[images][0]='+escape('https://quran.ksu.edu.sa/images_home/quran_img_th.png');
                SG.Plugins.Tools.openurl(url, this);

            });
            $("#tls_share").on(END_EV, function(e){
                var sura = currAya.split('_')[0];
                var aya = currAya.split('_')[1];//
                var aya_id = aya2id(sura, aya);
                $.ajaxSetup({ async: false });
                ...
                SG.Plugins.DBP.search('id', aya_id, tash, function (json) {
                    var nass = "{" + json[sura + '_' + aya] + "}" + " " + '[' + QuranData.Sura[sura][4] + " : " + aya + ']';
                    nass += "\n\n" + _lang['ayaLink'] + ": " + 'https://quran.ksu.edu.sa/index.php?aya=' + sura + '_' + aya;

                    SG.Plugins.Tools.send('', nass);
        ...
        postInit: function () {
            ...
            $("#browse_sel_sura,#tafsir_sel_sura,#repeat_sel_begin_sura,#repeat_sel_end_sura,#mtest_sel_begin_sura,#mtest_sel_end_sura,#m_sel_begin_sura,#m_sel_end_sura").on("touchstart", function(e){
                    var $this = $(this);
            		setTimeout(function(){
    ...
    SG.DimMan = (function ($) { // central dimensions manager
        ...
        function adjustDims(toggleZoom) {
            ...
            var tx = w / 12;
            if (doZoom === null) {
                doZoom = (SG.Initer.currZoom != null) ? parseInt(SG.Initer.currZoom,10) : (window.innerWidth <= 480);
            }
            if (toggleZoom) {
                doZoom = !doZoom;////
    ...
    function showCopyWidg(){
        ...
        function galber(){
            ...
            galb(ids, sura, firstAya, aya, function(nass){
                a.tm();
                nass += "\n\n" + _lang['ayaLink'] + ": " + 'https://quran.ksu.edu.sa/index.php?aya=' + sura + '_' + firstAya;
                orgNass = nass;
                $("#fld_copy").val(nass);
            });
        }
...
SG.Dialog = (function($){
    ...
    function modal(cb,config){
        ...
        $p_over.on('touchmove', function (e) {
            e.preventDefault();
            e.stopPropagation();
        });
        $p_modal.on('touchmove', function (e) {
            e.preventDefault();
        });     
        $p_over.on("tapone", function(){
	        	if(! persist){
        ...
        $p_modal.setGroups = function(groups){
            ...
            $p_modal.find(".groups").width(Math.round(groups_w*1.05));
